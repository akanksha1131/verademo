 ...
 */
 ...
 predicate isSource(DataFlow::Node src, string sourceType, string sourceName, string sourceDataType) {
   exists(RequestMappingMethod method, Parameter p |
     ...
     p.getName() in ["httpRequest", "request"] and
    //  p.getType() instanceof RefType and
    //  p.getType().(RefType).hasQualifiedName("javax.servlet.http", "MultipartHttpServletRequest") and
 ...
 select source,
   "Source's Package: " + source.getEnclosingCallable().getDeclaringType().getPackage().getName() +
     ", class: " + source.getEnclosingCallable().getDeclaringType().getName() + ", function: " +
     source.getEnclosingCallable().getName() + ", signature: " +
     source.getEnclosingCallable().getQualifiedName() + "(" +
     source.getEnclosingCallable().getSignature() + ")" + ". Source type: " + sourceType +
     ", Source name: " + sourceName + ", Source data type: " + sourceDataType
