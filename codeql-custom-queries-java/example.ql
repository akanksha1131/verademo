 ...
 */
 ...
 // Query to output each detected source with its package, class, function declaration, and signature, including the entire source function body
 ...
 select 
   ...
   ", signature: " + source.getEnclosingCallable().getSignature(),
   source.getEnclosingCallable().getBody().toString()
